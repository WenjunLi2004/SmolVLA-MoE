...
from .smolvla_model import SmolVLAMoE
...
def encode_obs(observation: Dict, model: SmolVLAMoE) -> Dict:
...
def get_model(usr_args: Dict) -> SmolVLAMoE:
    ...
    model = SmolVLAMoE.from_pretrained(str(policy_path))
    model.to(device)
    model.eval()
    model.reset()
...
def eval(TASK_ENV: Any, model: SmolVLAMoE, observation: Dict) -> None:
